...
def get_r_terms(events: Events, beta: th.Tensor) -> th.Tensor:
    ...
    labels = th.argmax(events.labels, dim=-1)  # [B,L] #Returns actual label.
    ...
    arg = - beta.unsqueeze(0).unsqueeze(0) * delta_t_i            # [B,L-1,M,N] #beta_{m,n}*(t_{i}-t_{i-1})
    ...
    ones = ones.unsqueeze(dim=2).repeat(1, 1, marks, 1)  # [B,L-1,M,N] #Not extremely clear
    ones = ones.type(exp.dtype)
    r_terms = [th.zeros(
        batch_size, marks, marks,
        dtype=exp.dtype, device=exp.device)] #[B,M,N]
