...
class SepCumulativeDecoder(VariableHistoryDecoder, abc.ABC):
    """Decoder based on Cumulative intensity method. Here, the cumulative
       ground intensity and the mark distribution are specified. The ground intensity is computed 
       as a derivate. 
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(SepCumulativeDecoder, self).__init__(
            name=name,
    ...
    def forward(
        ...
        intensity_integrals_q, p_m, intensity_mask_q, artifacts = \
            self.cum_intensity(
                events=events,
                query=query,
        ...
        if self.do_zero_subtraction: 
            (intensity_integrals_z, _, intensity_mask_z,
        ...
        if th.isnan(intensity_integrals).any():
            print(intensity_integrals[:,:,0])

        check_tensor(intensity_integrals * intensity_mask.unsqueeze(-1),
                     positive=True)

        
        intensity = th.autograd.grad(
            outputs=intensity_integrals,
            inputs=query,
            grad_outputs=th.ones_like(intensity_integrals),
            retain_graph=True,
            create_graph=True)[0]
        query.requires_grad = False


        intensity.data.clamp_(min=1e-5, max=1e8)
        '''
        if th.isnan(intensity).any():
            id_max = th.argmax(intensity_integrals)
            id_min = th.argmin(intensity_integrals)

            print(th.max(intensity_integrals), 'Max intensity integrals')
            print(th.min(intensity_integrals), 'Min intensity integrals')
            print(marked_intensity.flatten()[id_min], 'intensity at id min')
            print(marked_intensity.flatten()[id_max], 'intensity at id max')
            print(intensity_integrals[th.isnan(marked_intensity)], 'nan intensity integrals')
        '''

        check_tensor(intensity, positive=True, strict=True)
        log = Log.apply
        
        intensity = intensity.unsqueeze(-1)                
        marked_intensity = intensity * p_m
        check_tensor(marked_intensity, positive=True, strict=True)

        marked_log_intensity = log(marked_intensity)
        
        #Trick to have a tensor of size [B,T,M] compatible with modular. Only the ground intensity integrals (sum) is required for the loss.  
        ones = th.ones_like(p_m)
        marked_intensity_integrals = (intensity_integrals / self.marks) * ones
