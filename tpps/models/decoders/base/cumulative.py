...
class CumulativeDecoder(VariableHistoryDecoder, abc.ABC):
    ...
    def forward(
        ...
        if self.do_zero_subtraction: #Computes Lambda(0)
            ...
            #Is this line added because integrals_z is sometimes greater than intensity integral q ? Violation of monotonicity ? 
            intensity_integrals_q = th.clamp(
                intensity_integrals_q - intensity_integrals_z, min=0.
            ) + intensity_integrals_z #What happens if we remove this line ? 
        ...
        if th.isnan(marked_intensity).any():
