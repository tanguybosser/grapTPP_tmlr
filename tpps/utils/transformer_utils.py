...
from tpps.pytorch.activations import ACTIVATIONS
from tpps.pytorch.activations import AdaptiveGumbel
from tpps.pytorch.activations import AdaptiveGumbelSoftplus
from tpps.pytorch.layers import LAYER_CLASSES
from tpps.pytorch.layers import BatchNorm1d, LayerNorm

from tpps.utils.multi_head_attention import MultiheadAttention
...
def generate_square_subsequent_mask_fixed(sz, hist_size, device):
    mask1 = (th.triu(th.ones(sz, sz))).transpose(0, 1).to(device)
    mask2 = th.triu(th.ones(sz, sz), -(hist_size-1)).to(device)
    mask = mask1 * mask2 
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        if self.normalisation == "layernorm_with_running_stats": #True by default 
