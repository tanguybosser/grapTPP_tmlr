...
def event_encoder(
    ...
    if encoding == "times_only":
        times = th.clamp(times, min=1e-6)
        return times.unsqueeze(-1), mask  # [B,L+1 or T,1]

    if encoding == "log_times_only":
        times = th.clamp(times, min=1e-6) #Change back to 1e-6 ? 
        times = times + (times == 0).float() * epsilon(
            dtype=times.dtype, device=times.device)
        return th.log(times).unsqueeze(-1), mask #[B, L+1 or T, 1]
    ...
    if encoding == 'log_concatenate':
        times = th.clamp(times, min=1e-6)
        merged_representations = th.cat(
            (embeddings, th.log(times).unsqueeze(-1)),
            dim=-1)  # [B,L+1,D+1], [B,L+1]
        return merged_representations, mask
