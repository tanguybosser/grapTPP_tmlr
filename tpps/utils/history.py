...
from tpps.utils import batch as bu
from tpps.utils.events import Events
from tpps.utils.utils import smallest_positive
from tpps.utils.index import take_2_by_2
from tpps.utils.history_bst import get_prev_times as get_prev_times_bst
...
def build_histories(
        ...
        allow_partial_history: Optional[bool] = False,
        neg_index_last=False, 
        aligned=False,
        allow_window=False,
        return_history=True) -> Tuple[th.Tensor, th.Tensor]:
    ...
    ((prev_times, prev_times_idxs),
     is_event, pos_delta_mask) = get_prev_times_bst(
        query=query, events=events, allow_window=allow_window)             # ([B,T], [B,T]), [B,T], [B,T]

    if aligned:
        relative_history_idxs = th.arange(
            start= 2- history_size, end=2, device=events.times.device)
    else:
        relative_history_idxs = th.arange(
    ...
    #if aligned:
    #   history_seq_idxs[history_seq_idxs >= 0] = history_seq_idxs[history_seq_idxs >= 0] + 1
       

    if neg_index_last:
        history_seq_idxs[history_seq_idxs < 0] = 1e10
        history_seq_idxs,_ = th.sort(history_seq_idxs, dim=-1)
        history_seq_idxs[history_seq_idxs == 1e10] = -1
    ...
    if not allow_partial_history:
        ...
        #history_idxs_null = th.any(history_seq_idxs == 0)
        history_idxs_positive = th.any(history_seq_idxs > 0, dim=-1)
        
        #history_idxs = history_idxs_positive * history_idxs_null
        #history_idxs_positive = th.any(history_seq_idxs >= 0, dim=-1)        
    history_idxs_positive = history_idxs_positive.type(pos_delta_mask.dtype)
    #Otherwise first event does not define a valid history. 
    #history_mask = pos_delta_mask * history_idxs_positive 
    history_mask = history_idxs_positive.type(history.dtype)  # [B,T]

    return history, history_mask, history_seq_idxs
