...
from tpps.utils.commands import get_command
...
parser.add_argument("--val-emb-dim", type=int, default=[4], nargs="+", metavar='N')
parser.add_argument("--val-encoder-units-rnn", type=int, default=[], nargs="+", metavar='N')
parser.add_argument("--val-encoder-layers-rnn", type=int, default=[], nargs="+", metavar='N')
parser.add_argument("--val-units-mlp", type=int, default=[], nargs="+", metavar='N')
parser.add_argument("--val-decoder-n-mixture", type=int, default=[], nargs="+", metavar='N')
parser.add_argument("--decoder-mc-prop-est", type=int, default=10)
parser.add_argument("--val-decoder-layers-rnn", default=[], nargs="+", metavar='N')
parser.add_argument("--val-decoder-units-rnn", default=[], nargs="+", metavar='N')
parser.add_argument("--val-n-heads", default=[], nargs="+", metavar='N')
...
parser.add_argument("--num-config", type=int, default=5)
parser.add_argument("--splits", default=[1,2,3,4,5], nargs="+", type=int)
...
parser.add_argument("--exp-name", default=None, type=str, 
                    help="Additional name to give the experiment. If None experiment will be saved under dataset + encoding + encoder + decoder. Defaults to None.")
parser.add_argument("--use-softmax", default=True, type=lambda x: strtobool(x), 
                    help="If True, applies softmax to intensity coefficients. Defaults to True.")
parser.add_argument("--lr", default=0.001, type=float, 
                    help="learning rate")

def random_search(param_dic, args, num_config=5):
    if len(param_dic) == 0:
        return [dict()]
    configs= []
...
if __name__ == '__main__':
    ...
    for i, dataset in enumerate(args.datasets):
        for split in args.splits:
            ...
            configs = random_search(param_dic, args, num_config=args.num_config)
            config_val_losses = []
            ...
            #print('\n TESTING BEST CONFIGURATION for {}-{}-{}-{} ON SPLIT : {}/{} \n'.format(dataset, args.encoding, args.encoder, args.decoder, split, num_splits))
            while True:
                    try:                    
